...
function obs_trial_screen(word, condition) {
  ...
  // Predefined corner positions using vw and vh units for responsive positioning
  ...
  for (let i = 0; i < 4; i++) {
    var snd_filename = "snd/" + word + "_" + shuffled_speakers[i] + ".wav";
...
function video_obs_trial(tStart, tEnd, vid, cond, first_speaker) {
  var video_filename = "vid/" + vid +"_" + cond + "_" + first_speaker + ".mp4";
  return {
...
function selectRandomCarrierPhrase(word) {
  ...
  const carriers = ["i_bet_thats", "im_guessing_its", "that_might_be"];
